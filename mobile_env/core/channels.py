...
class Channel:
    ...
    def isoline(cls, bs, ue_config, map_bounds, dthresh, num=32):
        """Compute points on isoline where UEs receive at least `dthres` max. data rate from BS."""
    ...
    def boundary_collison(cls, theta, x0, y0, width, height):
        ...
        lwr_x1, lwr_y1 = np.tan(theta - 1/2 * np.pi) * \
            (y0 - 0.0) + x0, 0.0
        ...
        if theta == 0.0:
            ...
            return np.min((rgt_x1, upr_x1, width)), np.min((rgt_y1, upr_y1, height))

        elif theta == 1/2 * np.pi:
            return x0, height

        elif theta > 1/2 * np.pi and theta < np.pi:
            return np.max((lft_x1, upr_x1, 0.0)), np.min((lft_y1, upr_y1, height))
